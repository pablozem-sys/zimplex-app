...
const inputClass = 'w-full bg-white border border-gray-200 rounded-2xl px-4 py-3.5 text-sm text-gray-900 focus:outline-none focus:ring-2 focus:ring-[#2D68F6] focus:border-transparent shadow-sm'
  ...
  if (success) {
    return (
      <div className="page-content flex flex-col items-center justify-center min-h-[60vh]">
        ...
        <p className="text-3xl font-bold text-[#2D68F6] mt-4">{fmt(lastTotal)}</p>
      </div>
    )
  }
    ...
    <div className="page-content">
      ...
      <div className="flex bg-gray-100 rounded-2xl p-1 mb-5">
        <button
          ...
          className={`flex-1 py-2.5 rounded-xl text-sm font-semibold transition-all ${tab === 'registrar' ? 'bg-white text-[#2D68F6] shadow-sm' : 'text-gray-400'}`}
        >
          Registrar
        </button>
        <button
          onClick={() => setTab('historial')}
          className={`flex-1 py-2.5 rounded-xl text-sm font-semibold transition-all ${tab === 'historial' ? 'bg-white text-[#2D68F6] shadow-sm' : 'text-gray-400'}`}
        >
          Historial
        </button>
      </div>
      ...
      {tab === 'registrar' ? (
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div>
            ...
            <div className="flex items-center gap-3">
              ...
              <input
                ...
                className="flex-1 bg-white border border-gray-200 rounded-2xl px-4 py-3.5 text-sm text-gray-900 text-center font-bold shadow-sm focus:outline-none focus:ring-2 focus:ring-[#2D68F6] focus:border-transparent"
              />
              <button type="button" onClick={() => setForm(f => ({ ...f, quantity: f.quantity + 1 }))}
                className="w-12 h-12 rounded-2xl bg-[#2D68F6] text-white font-bold text-xl flex items-center justify-center active:scale-95 shadow-md shadow-blue-200">
                +
              </button>
            </div>
          </div>
                ...
                <button
                  ...
                  className={`py-3.5 rounded-2xl border text-sm font-medium flex flex-col items-center gap-1 transition-all shadow-sm ${
                    form.paymentMethod === pm.id
                      ? 'border-[#2D68F6] bg-blue-50 text-[#2D68F6] shadow-blue-100'
          ...
          <button
            type="submit"
            className="w-full bg-[#2D68F6] active:scale-[0.98] text-white font-semibold py-4 rounded-2xl transition-all duration-200 shadow-lg shadow-blue-200 text-base"
          >
            Registrar venta
          </button>
        </form>
              ...
              {daySales.map(sale => (
                <div key={sale.id} className="bg-white border border-gray-100 rounded-2xl px-4 py-3.5 flex items-center gap-3 shadow-sm">
                  <div className="w-9 h-9 rounded-2xl bg-blue-50 flex items-center justify-center flex-shrink-0">
                    <TrendingUp size={15} className="text-[#2D68F6]" />
                  </div>
