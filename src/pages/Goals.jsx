    ...
    <div className="fixed inset-0 bg-black/40 z-50 flex items-end justify-center" onClick={onClose}>
      <div className="bg-white w-full max-w-[430px] rounded-t-[32px] p-6 pb-10" style={{ marginBottom: '84px' }} onClick={e => e.stopPropagation()}>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <button type="submit"
            className="w-full bg-[#6366F1] text-white font-semibold py-4 rounded-2xl active:scale-[0.98] transition-all mt-2 shadow-lg shadow-indigo-200">
            Crear meta
          </button>
        </form>
      </div>
    ...
    <div className="fixed inset-0 bg-black/40 z-50 flex items-end justify-center" onClick={onClose}>
      <div className="bg-white w-full max-w-[430px] rounded-t-[32px] p-6 pb-10" style={{ marginBottom: '84px' }} onClick={e => e.stopPropagation()}>
        ...
        <div className="flex gap-2 mb-5">
          {suggestions.map(s => (
            <button key={s} onClick={() => setAmount(s)}
              className={`flex-1 py-2.5 rounded-2xl text-xs font-semibold transition-all ${parseInt(amount) === s ? 'bg-[#6366F1] text-white shadow-md shadow-indigo-200' : 'bg-gray-100 text-gray-600'}`}>
              +{s >= 1000 ? `${s / 1000}k` : s}
            </button>
          ))}
        </div>
        ...
        <button onClick={() => { onAdd(goal.id, parseInt(amount) || 0); onClose() }}
          ...
          className="w-full bg-[#6366F1] disabled:bg-gray-200 disabled:text-gray-400 text-white font-semibold py-4 rounded-2xl active:scale-[0.98] transition-all shadow-lg shadow-indigo-200 disabled:shadow-none">
...
export default function Goals() {
  ...
  return (
    <div className="page-content">
      ...
      {goals.length > 0 && (
        <div className="bg-gradient-to-br from-[#6366F1] to-[#818CF8] rounded-[24px] p-5 mb-5 text-white shadow-xl shadow-indigo-200">
...
function GoalCard({ goal, onAdd, onDelete }) {
  ...
  const barColor = done ? '#7CD09B' : progress > 60 ? '#6366F1' : progress > 30 ? '#818CF8' : '#DC4B56'

  return (
    <div className="bg-white border border-gray-100 rounded-[20px] p-4 shadow-sm">
      <div className="flex items-start justify-between mb-3">
        <div className="flex items-center gap-3">
          <div className={`w-11 h-11 rounded-2xl flex items-center justify-center ${done ? 'bg-green-50' : 'bg-indigo-50'}`}>
            <Target size={18} className={done ? 'text-[#7CD09B]' : 'text-[#6366F1]'} />
          </div>
          <div>
      ...
      {!done && (
        <button onClick={onAdd}
          className="w-full bg-gray-50 border border-gray-200 text-gray-600 text-sm font-semibold py-3 rounded-2xl active:scale-95 transition-all hover:bg-indigo-50 hover:border-blue-200 hover:text-[#6366F1]">
          + Agregar progreso
        </button>
      )}
    </div>
